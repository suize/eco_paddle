...
def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open('data_splits/classInd.txt')]
    ...
    splits = []
    for i in range(1, 4):
        train_list = [line2rec(x) for x in open('data_splits/trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open('data_splits/testlist{:02d}.txt'.format(i))]
